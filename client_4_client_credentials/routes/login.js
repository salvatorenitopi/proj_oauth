	...
	var post_req = https.request(options, function(post_res) {
		...
		post_res.on('data', function (body) {
			...
			if (jdata.status == "error"){
				...
				if (jdata.access_token) {	// Se il token è valido
					...
					res.end ( JSON.stringify({"status": "error", "message": "access_token non fornito o non valido" }) );
...
router.get(config.end_login, function(req, res){
	...
	if (req.session.login && req.session.access_token && req.session.login === "YES") {
		...
		if (login){
			...
			res.render('login', { "authorize_url": "/login?login=true" });
		}
